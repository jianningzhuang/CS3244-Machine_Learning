...
def perceptron(data, labels, params={}, hook=None):
    ...
    #print("d = {}, n = {}, theta = {}, theta_0 = {}".format(d,n,theta,theta_0))
    for t in range(T):     
      for i in range(n):
        y = labels[0][i]
        x = data[:,i:i+1]
        a = np.asscalar(np.dot(theta.T,x)+theta_0)
        #print("a = {}".format(a))
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    ...
    (d, n) = data_test.shape
    total = score(data_test, labels_test, th, th0)
    return total/n
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for i in range(it):
        ...
        total += eval_classifier(learner, data_train, labels_train, data_test, labels_test)
    return total/it
...
def xval_learning_alg(learner, data, labels, k):
    ...
    for i in range(k):
        if i == 0:
            data_train = np.concatenate(data_split[i+1:], axis = 1)
            labels_train = np.concatenate(labels_split[i+1:], axis = 1)
        elif i == k-1:
            data_train = np.concatenate(data_split[:i], axis = 1)
            labels_train = np.concatenate(labels_split[:i], axis = 1)
        else:
            data_train1 = np.concatenate(data_split[:i], axis = 1)
            data_train2 = np.concatenate(data_split[i+1:], axis = 1)
            data_train = np.concatenate((data_train1, data_train2), axis = 1)
            labels_train1 = np.concatenate(labels_split[:i], axis = 1)
            labels_train2 = np.concatenate(labels_split[i+1:], axis = 1)
            labels_train = np.concatenate((labels_train1, labels_train2), axis = 1)
        data_test = data_split[i:i+1][0]
        labels_test = labels_split[i:i+1][0]
        total += eval_classifier(learner, data_train, labels_train, data_test, labels_test)
    return total/k
...
print(eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=0.25), 20, 20, 1000))
